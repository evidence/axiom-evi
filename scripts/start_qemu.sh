...
if  [ $NETWORK -eq 1 ]; then
    if2exist $TAPNAME$ID || { echo "interface $TAPNAME$ID does not exist! have you run 'make netup' on host?"; exit 255; }   
...
if [ "$BOARD" = "zynq" ]; then
    ${QEMU} -M arm-generic-fdt-plnx -machine linux=on -m 256            \
