            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        /* if ((dst_interface == i) && ((msg_cmd == AXIOM_DSCV_CMD_RSP_NOID) || (msg_cmd == AXIOM_DSCV_CMD_RSP_ID))) */
                                    ...
                                    else
                                    {
                                        ...
                                        while (msg_cmd != AXIOM_DSCV_CMD_END_TOPOLOGY)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    /* Immediately update my routing table: src_node_id node is connceted to my 'dst_interface' interface */
                                                    /* b_mask = axiom_codify_routing_mask(dev, src_node_id, dst_interface);
                                                       axiom_set_routing(dev, src_node_id, b_mask); */

                                                    /* Reply 'I am node 'my_node_id', I'm on interface 'dst_interface' */
                     ...
                     axiom_node_id_t *my_node_id)
{
    ...
    axiom_node_id_t src_node_id, dst_node_id;
    axiom_if_id_t src_interface, data_src_if, dst_interface, data_dst_if;
    int i, j, w;
    uint8_t b_mask;
    axiom_msg_id_t ret;
    ...
    *my_node_id = axiom_get_node_id(dev);
    ...
    if (*my_node_id == 0)
    {
        DPRINTF("Slave: send to interface number = %d the AXIOM_DSCV_CMD_RSP_NOID message", dst_interface);

        /* Reply 'I do not have an id, I'm on interface dst_interface' */
            ...
            else
            {
                ...
                *my_node_id = dst_node_id;
                ...
                else
                {
                    ...
                    DPRINTF("Node:%d Ended discovery phase, send my topology table to _node:%d on my interface:%d", *my_node_id, src_node_id, dst_interface);
                    ...
                    DPRINTF("Node:%d Send AXIOM_DSCV_CMD_END_TOPOLOGY message to Node:%d on my interface:%d", *my_node_id, src_node_id, dst_interface);
